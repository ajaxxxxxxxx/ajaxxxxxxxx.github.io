...
Absolute Wrath is the patrician's choice for a more challenging Listo experience, and will enable _random_ buffs to be applied to enemies. These buffs range from mild (resistance to common damage types) to wild (explodes on death in a cloud that disarms and confuses anyone nearby while triggering a wild magic surge). Listo's CX config intentionally applies a number of curated choices from AW to various enemies; enabling Absolute Wrath will enable _both_ the original purpose of the mod (random distribution of modifiers), and the part of Listo's CX config that hands out AW modifiers. tl;dr even if no random effects are assigned in a particular fight, Listo will probably be harder - consider this the "Wild Wasteland" experience.
