...
- Listonomicon incorporates Zerd's RAW rules for attunement, relaxed to a 5 attuned magic item limit. This means that each individual character can only use and equip up to 5 attunement requiring items. This is intended to limit end game stacking of every single slot with powerful items, it also allows some lesser magic items to shine as the ones that do not require attunement will be able to fit into builds where stronger, attunement-requiring items can't.
